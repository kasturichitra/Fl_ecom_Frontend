...
const Dashboard = () => {
  ...
  return (
    <div className="flex flex-col gap-6">
      {/* --- TABS HEADER --- */}
      <div className="flex gap-4 border-b pb-2 overflow-x-auto whitespace-nowrap scrollbar-hide">
        {["overall", "performance", "inventory"].map((tab) => (
          <button
            key={tab}
            onClick={() => setActiveTab(tab)}
            className={`px-4 py-2 text-sm font-medium rounded-t-md transition shrink-0 ${activeTab === tab ? "bg-blue-600 text-white shadow" : "text-gray-600 hover:bg-gray-100"
              }`}
          >
